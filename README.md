<h1 align="center">Ideogram V3 MCP Server</h1>

<p align="center">
  <strong>Create Ideogram V3 AI tasks through RunAPI — 4 endpoints, 7 models — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Models-7-green?style=flat-square" alt="7 models">
</p>
...
`@runapi.ai/ideogram-v3-mcp` is a focused MCP server for the **Ideogram V3** model line on RunAPI.
It lets an MCP-compatible assistant create Ideogram V3 tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `edit_image` | Yes | Create a Ideogram V3 task (edit image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `reframe_image` | Yes | Create a Ideogram V3 task (reframe image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `remix_image` | Yes | Create a Ideogram V3 task (remix image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `text_to_image` | Yes | Create a Ideogram V3 task (text to image) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a Ideogram V3 model and endpoint. |

---

## Models

Ideogram V3 covers 7 models across 4 endpoints. Each tool accepts the models listed for it:
